...
use ggml::prelude::{GOpPool, GType};
use rusty_ggml::prelude::{GContext, GTensor2};
...
struct MnistModel {
        pub conv2d_1_kernel: GTensor2,
        pub conv2d_1_bias: GTensor2,
        pub conv2d_2_kernel: GTensor2,
        pub conv2d_2_bias: GTensor2,
        pub dense_weight: GTensor2,
        pub dense_bias: GTensor2,
        pub ctx: GContext,
}
...
fn mnist_model_load(model_path: String) -> Result<MnistModel> {
        ...
        let ctx: GContext = gguf_context.get_ggml_context();
...
fn mnist_eval(model: &MnistModel, n_threads: usize, digit: &[f32], fname_cgraph: Option<&str>) -> Result<i32> {
        ...
        cur = cur.reshape([1600, 1]);
        // Final Dense layer
        cur = model.dense_weight.mul_mat(&cur).add(&model.dense_bias);
        ...
        // let prediction = std::max_element(probs_data, probs_data + 10) - probs_data;
