...
use crate::dims::{Dim, DimGtE, DimLt, DimPair, DimValid};
use crate::gtensor::{GTensor, GTensorError};
use crate::prelude::GOpPool;
use crate::util::GType;
use crate::validation::GMemoryRequest;

macro_rules! mk_simple_uops {
  ( $($(#[$attr:meta])* [$opname:ident, $gfname:ident]),* $(,)? ) => { $(
    $(#[$attr])*
    pub fn $opname<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T) -> GTensor<DIMS>
    where
        Dim<DIMS>: DimValid
    {
        let tensor: &GTensor<DIMS> = tensor.as_ref();
        self.new_unary(tensor, |ctx, ictx, tptr| {
            let mr = GMemoryRequest::estimate_tensor_request_ictx(
                ctx, ictx, tensor.md.typ, tensor.md.shape
            ).fit_or_die()?;
...
impl GContext
{
    ...
    pub fn norm<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T, eps: f32) -> GTensor<DIMS>
        where
            Dim<DIMS>: DimValid
    {
        ...
        self.new_unary(tensor, |ctx, ictx, tptr| {
            let mr =
                GMemoryRequest::estimate_tensor_request_ictx(ctx, ictx, tensor.md.typ, tensor.md.shape)
                    .fit_or_die()?;
    ...
    pub fn rms_norm<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T, eps: f32) -> GTensor<DIMS>
        where
            Dim<DIMS>: DimValid
    {
        let tensor = tensor.as_ref();
    ...
    /// ```
    pub fn mean<const ODIMS: usize, const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T) -> GTensor<ODIMS>
        where
            Dim<DIMS>: DimValid,
            Dim<ODIMS>: DimValid,
    ...
    /// assert_eq!(result, [10]);
    /// ```
    pub fn sum<const ODIMS: usize, const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T) -> GTensor<ODIMS>
        where
            Dim<DIMS>: DimValid,
            Dim<ODIMS>: DimValid,
            DimPair<ODIMS, 2>: DimLt,
    {
        let tensor = tensor.as_ref();
        self.new_unary(tensor, |ctx, ictx, tptr| {
            let mr = GMemoryRequest::estimate_tensor_request_ictx(ctx, ictx, tensor.md.typ, [1])
                .fit_or_die()?;
    ...
    pub fn reshape<const ODIMS: usize, const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T, ne: [usize; ODIMS]) -> GTensor<ODIMS>
        where
            Dim<DIMS>: DimValid,
            Dim<ODIMS>: DimValid,
            ...
            DimPair<ODIMS, 5>: DimLt,
    {
        ...
        self.new_unary(tensor, |ctx, ictx, tptr| {
            ...
            let mr = GMemoryRequest::estimate_tensor_request_ictx(ctx, ictx, tensor.md.typ, shp)
                .fit_or_die()?;
            Ok((
                mr,
                match ODIMS {
    ...
    /// # !!!! FIXME !!!!
    /// # !!!! FIXME !!!!
    pub fn view<const ODIMS: usize, const DIMS: usize, T: AsRef<GTensor<DIMS>>>(
        &self,
        tensor: T,
        ne: [i64; ODIMS],
        offset: [usize; ODIMS],
    ...
    /// # !!!! FIXME !!!!
    /// # !!!! FIXME !!!!
    pub fn diag_mask_inf<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T, val: usize) -> GTensor<DIMS>
        where
            Dim<DIMS>: DimValid
    {
        let tensor = tensor.as_ref();
        self.new_unary(tensor, |ctx, ictx, tptr| {
            ...
            let mr1 = GMemoryRequest::estimate_tensor_request_ictx(ctx, ictx, tensor.md.typ, []);
    ...
    /// # !!!! FIXME !!!!
    /// # !!!! FIXME !!!!
    pub fn rope<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(&self, tensor: T, n_past: usize, n_dims: usize, mode: usize, n_ctx: usize) -> GTensor<DIMS>
        where
            Dim<DIMS>: DimValid
    {
        let tensor = tensor.as_ref();
    ...
    /// # !!!! FIXME !!!!
    /// # !!!! FIXME !!!!
    pub fn rope_custom<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(
        &self,
        tensor: T,
        n_past: usize,
        n_dims: usize,
    ...
    pub fn pool_2d<const DIMS: usize, T: AsRef<GTensor<DIMS>>>(
        &self,
        tensor: T,
        op: GOpPool,
        k0: i32,
